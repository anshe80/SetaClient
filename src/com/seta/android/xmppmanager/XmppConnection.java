...
public class XmppConnection {
	...
	public static XMPPConnection openConnection(Activity context) {
		try {
			if (context != null
				...
				OpenfileFunction.makeFilePath(Environment
						.getExternalStorageDirectory().getPath()
						+ "/seta/security/", "cacerts.bks");
				// delete by anshe 2015.5.26
				/*
				 * File file =new
				 * File(Environment.getExternalStorageDirectory().
				 * getPath()+"/seta/security/"); if(!file.exists()){
				 * file.mkdirs(); }
				 */
				// delete by anshe 2015.5.26
				config.setTruststorePath(Environment
						.getExternalStorageDirectory().getPath()
						+ "/seta/security/cacerts.bks");
	...
	private static void configureConnection(ProviderManager pm) {
		...
		// Service Discovery # Items //解析聊天室列表
		pm.addIQProvider("query", "http://jabber.org/protocol/disco#items",
				new DiscoverItemsProvider());
		// Service Discovery # Info //某一个聊天室的信息
